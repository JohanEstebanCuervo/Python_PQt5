...
def ext_masks(file, listing):
    masks=[]
    for name in sorted(listing):
        a=cv2.cvtColor(cv2.imread(file+"/"+name), cv2.COLOR_BGR2GRAY) #lectura de imagen, transformacion a escala de grises
        [T, mask] = cv2.threshold(a, 0, 255, cv2.THRESH_BINARY | cv2.THRESH_OTSU) #binarizar mask con algotirmo de OTSU
        masks.append(np.array(mask))  #Se agrega la mask a una listing de arrays
    return masks
...
def Write_Variable(name,variable):
    fichero = open('Resultados\Variables/'+name+'.pickle','wb')
...
def Read_Variable(name):
    ...
    listing_fichero = pickle.load(fichero)
    
    fichero.close()
    
    return listing_fichero
    
def Imagenes_Camara(file, listing,masks, color_check ): 
    espectro=[]
    entrada=[]
    colorn=[[0, 0, 0]]
    prom=[]
    ...
    for name in sorted(listing):
        a=name.find('(')+1
        espectro.append(int(name[a:a+3]))
        imagen=cv2.cvtColor(cv2.imread(file+"/"+name), cv2.COLOR_BGR2GRAY)#cargamos imagenes multiespectrales en escala de grises
        ...
        for i in range(len(masks)): #se recorren las masks
            parte=imagen[np.where(masks[i]==255)] #es extrae los pixeles de cada parche
...
def Pesos_ecualizacion(imagenes_patron, mask, valor_ideal=243,shape_imag=(480,640)):
    promedios=[]
...
def Error_de_reproduccion(imagenes_RGB, masks, color_check):
    ...
    for imagen in imagenes_RGB:
        ...
        for i,mask in enumerate(masks):
            indices=np.where(mask.reshape(-1,)==255)
...
def comparacion_color_check(name,im_RGB,color_check_RGB,masks,file=''):
    ...
    imshow(file + '/Comparación Color_Check - '+ name,imagen.astype(int))
    imwrite(file + '/Comparacion Color_Check - '+ name+'.png',imagen/255)
...
def mejor_combinacion(imagenes_patron,masks,color_check,Cant_Image,type_error='mean',imagen_write='off'):
